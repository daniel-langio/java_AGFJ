...
public class AnimatedEntity extends TexturedEntity {
    ...
    @Setter
    private AnimationCategory currentAnimationCategory = DEFAULT_ANIMATION_CATEGORY;
    @Setter
    ...
    public SpriteAnimation getCurrentAnimation() {
        SpriteAnimation currentAnimation = animations.stream()
            .filter(a -> a.getCategory().equals(currentAnimationCategory)).findFirst().orElse(null);

        if (currentAnimation == null) {
            throw new IllegalArgumentException(String.format("Animation category %s not found", currentAnimationCategory));
        }
    ...
    public SpriteAnimation getDefaultAnimation() {
         SpriteAnimation defaultAnimation = animations.stream()
            .filter(a -> a.getCategory().equals(DEFAULT_ANIMATION_CATEGORY)).findFirst().orElse(null);
    ...
    public void setAnimations(List<SpriteAnimation> animations) {
        if (animations.stream().noneMatch(a -> REQUIRED_ANIMATION_CATEGORIES.contains(a.getCategory()))) {
            throw new IllegalArgumentException(
                String.format("This entity requires an animation for : %s", REQUIRED_ANIMATION_CATEGORIES));
        }
